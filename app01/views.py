...
def reg(request):
    ...
    if request.method == "POST":
        ...
        v_password = request.POST.get("v-password", "")
        if account == "" or password == "":
            info = "用户名或密码不能为空"
            return render(request, "regedit.html", {"info": info})
        if password != v_password:
            info = "两次密码不一致，请重新输入"
            return render(request, "regedit.html", {"info": info})

        exists = Administrator.objects.filter(account=account).exists()
        if exists:
            info = "用户名已经存在"
            return render(request, "regedit.html", {"info": info})
        else:
            d = dict(account=account, password=password)
            Administrator.objects.create(account=account, password=password)
            info = "用户创建成功"
            return render(request, "regedit.html", {"info": info})
# 车牌识别
def Park_discern(image):
        APP_ID = '33563516'
        API_KEY = 'sMGTwAwDKHEFCMQDKqxWItGa'
        SECRET_KEY = 'MnEMAVmCfKrwTZQSkyGaCs190oTvZTtG'
        # 创建客户端对象
...
@vip_valid
def code(request):
    img = qrcode.make('https://www.aiit.edu.cn/')  # 传入网站计算出二维码图片字节数据
    ...
    with open('../static/imgs/2-D barcode.png') as f:
        img.save(f)
    return response
@login_required
@vip_valid
